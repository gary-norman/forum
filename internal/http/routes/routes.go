...
func (r *RouteHandler) Routes() http.Handler {
	...
	userDataServer := http.FileServer(http.Dir(" e./db/userdata"))
	...
	mux.Handle("GET /", mw.WithUser(http.HandlerFunc(s.Home.GetHome), s.App))
	mux.Handle("GET /channels/{channelId}", mw.WithUser(http.HandlerFunc(s.Channel.GetThisChannel), s.App))
	mux.HandleFunc("GET /posts/create", s.Post.CreatePost)
	mux.HandleFunc("GET /search", s.Search.Search)
	mux.Handle("GET /posts/{postId}", mw.WithUser(http.HandlerFunc(s.Post.GetThisPost), s.App))
	mux.Handle("GET /users/{userId}", mw.WithUser(http.HandlerFunc(s.User.GetThisUser), s.App))
